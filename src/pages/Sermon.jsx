        ...
        <div className="p-6">
          ...
          <div className="space-y-4 mb-6">
            <div>
              ...
              <p className="text-gray-600">{sermon.bibleText}</p>
            </div>
            ...
            <div>
              ...
              <p className="text-gray-600">{sermon.minister}</p>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">
              Sermon Notes & Key Points:
            </h3>
            <div className="text-gray-700 leading-relaxed">
              {formatSermonNote(sermonsermonNote)}
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-white/10 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-md w-full p-6">
        ...
        <div className="text-center mb-4">
          ...
          <p className="text-gray-600">{sermon.minister}</p>
          <p className="text-sm text-gray-500">{sermon.bibleText}</p>
        </div>
...
const Sermon = () => {
  ...
  const categories = [
    "All",
    ...new Set(audioFiles.map((sermon) => sermon.category)),
  ];
  const years = ["All", ...new Set(audioFiles.map((sermon) => sermon.year))];

  // Filter sermons based on search and filters
  const filteredSermons = [...audioFiles] // create a copy to avoid mutating the original
    .reverse() // show latest first
    .filter((sermon) => {
      const matchesSearch =
        sermon.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
        sermon.minister.toLowerCase().includes(searchTerm.toLowerCase());

      const matchesCategory =
        selectedCategory === "All" || sermon.category === selectedCategory;
      const matchesYear =
        selectedYear === "All" || String(sermon.year) === selectedYear;
              ...
              >
                <div className="relative">
                  ...
                  <div className="absolute top-4 left-4 bg-white rounded-lg p-2 text-center shadow-md">
                    {(() => {
                      const { day, month } = getDayAndMonth(sermon.date);
                      return (
                        <>
                ...
                <div className="p-6">
                  ...
                  <p className="text-gray-600 mb-1">
                    <strong>Scripture:</strong> {sermon.bibleText}
                  </p>
                  <p className="text-gray-600 mb-4">
                    <strong>Minister:</strong> {sermon.minister}
                  </p>
